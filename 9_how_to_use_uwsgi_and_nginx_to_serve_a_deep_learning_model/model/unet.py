...
class UNet(BaseModel):
    ...
    def _set_training_parameters(self):
        ...
        self.validation_steps = (
            self.info.splits["test"].num_examples
            // self.batch_size
            // self.val_subsplits
        )
    ...
    def build(self):
        ...
        up_stack = [
            pix2pix.upsample(
                self.config.model.up_stack.layer_1, self.config.model.up_stack.kernels
            ),  # 4x4 -> 8x8
            pix2pix.upsample(
                self.config.model.up_stack.layer_2, self.config.model.up_stack.kernels
            ),  # 8x8 -> 16x16
            pix2pix.upsample(
                self.config.model.up_stack.layer_3, self.config.model.up_stack.kernels
            ),  # 16x16 -> 32x32
            pix2pix.upsample(
                self.config.model.up_stack.layer_4, self.config.model.up_stack.kernels
            ),  # 32x32 -> 64x64
        ]
    ...
    def train(self):
        ...
        trainer = UnetTrainer(
            self.model, self.train_dataset, loss, optimizer, metrics, self.epoches
        )
        trainer.train()
